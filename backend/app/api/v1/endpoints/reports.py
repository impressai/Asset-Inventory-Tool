...
@router.get("/summary")
def asset_summary(db: Session = Depends(get_db), current_user=Depends(require_admin_or_manager)):
...
def by_category_breakdown(db: Session = Depends(get_db), current_user=Depends(require_admin_or_manager)):
...
def assigned_vs_available(db: Session = Depends(get_db), current_user=Depends(require_admin_or_manager)):
